...
class Machine():
	...
	def __init__(self, IState, tape, brank, tag):
		self.state = IState
		self.oldState = None
		self.staked = 0
	...
	def print(self):
			print('Maquina', self.tag)
			self.tape.print()
			print("estado: ",self.state.cod)

	def copy(self):
		aux = Machine(self.state,self.tape, self.tape.brank, (self.tag + '-' + str(self.child)))
		self.child += 1
		return aux
	pass
...
if(sys.argv.__len__() >= 2):
	...
	for st in states:
		if(st.cod == IState):
			...
			break
	maq = []
	maq.append(Machine(IState, sys.argv[2], brank, '1'))
	saida = 1
	tam = 1
